...
- Mark a dataset `"exhaustive": false` when its official export is not the full population an OSM query returns. It only changes how `onlyInOsm` is presented, never matching. Narrowing a selector instead can misfire: an object the strict criteria exclude but that lacks no expected tag is rescued by neither pass and is reported missing. Prefer it over an `exclude` whose only job is tidying the OSM-only bucket.
- Expect a tag only where it says something. Mapping a source value that restates an OSM default (`covered=no` on an ordinary bike rack) or a placeholder for "unknown" (a `0` clearance) turns nearly every match into a tag difference; leave such values unmapped, or filter them with `extract`.
- A tag an `anyOf` branch requires is not an expectation — check `tagMapping.fixed` is not putting one back, or objects satisfying a different branch are told to add a tag they do not need.
- Before filtering an official source on a category column, check what other groups it holds: an `=` filter can silently drop an accessible variant that `LIKE` keeps and a `fromProps` rule turns into `wheelchair=yes`.
